/**
 * Export 1.5×0.75m street banner at print resolution.
 * Usage: node leaflet-mockups/print/export-banner.mjs
 *
 * Writes:
 *   out/banner-2x1.pdf  — send this to the printer (1500 × 750 mm, 200 DPI)
 *   out/banner-2x1.png  — same artwork as a 200 DPI RGB PNG
 *
...
async function equaliseWeekendBoxes(page) {
  await page.evaluate(() => {
    const boxes = [...document.querySelectorAll(".weekend")];
    if (boxes.length < 2) return;
    boxes.forEach((box) => {
      box.style.height = "auto";
    });
    const height = Math.max(
      ...boxes.map((box) => box.getBoundingClientRect().height),
    );
    boxes.forEach((box) => {
...
async function capturePrintPng(dpi) {
  ...
  try {
    ...
    const url = pathToFileURL(path.join(__dirname, "banner-2x1.html")).href;
    await page.goto(url, { waitUntil: "networkidle0", timeout: 180000 });
    await page.evaluate(() => document.fonts.ready);
    await new Promise((r) => setTimeout(r, 800));
    await equaliseWeekendBoxes(page);
...
try {
  ...
  console.warn(`200 DPI capture failed (${err.message}); retrying at 150 DPI`);
  usedDpi = 150;
  pngBuffer = await capturePrintPng(150);
}

const pngPath = path.join(outDir, "banner-2x1.png");
const pdfPath = path.join(outDir, "banner-2x1.pdf");

const printPng = await sharp(pngBuffer)
  .resize(Math.round((PAGE_W_MM / 25.4) * usedDpi), Math.round((PAGE_H_MM / 25.4) * usedDpi), {
    fit: "fill",
...
try {
  ...
  if (err.code === "EBUSY" || err.code === "EPERM") {
    const fallback = path.join(outDir, "banner-1.5x075.pdf");
    await writeFile(fallback, pdf);
    console.warn(`Could not overwrite banner-2x1.pdf (${err.code}); wrote ${path.basename(fallback)}`);
  } else {
    throw err;
  }
}
...
console.log(
  `Wrote banner-2x1.pdf and banner-2x1.png (${meta.width}×${meta.height} px, ${usedDpi} DPI, ${(printPng.length / 1e6).toFixed(1)} MB PNG).`,
);
console.log(
  "Done. Upload out/banner-2x1.pdf to a banner printer at 1500 × 750 mm (with hem).",
);
